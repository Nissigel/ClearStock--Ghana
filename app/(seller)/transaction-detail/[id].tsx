...
export default function SellerTransactionDetailScreen() {
  ...
  const isPendingFulfillment = transaction.status === 'PENDING_FULFILLMENT';
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        >
          {[
            { label: 'Buyer', value: transaction.buyer.fullName },
            {
              label: 'Buyer Phone',
              value: transaction.buyer.phoneNumber ?? 'Not revealed yet',
            },
            { label: 'Quantity', value: `${transaction.quantity} units` },
            {
              label: 'Price Per Unit',
              value: `${CURRENCY_SYMBOL}${transaction.priceAtCreation.toFixed(2)}`,
            },
            {
              label: 'Total Amount',
        ...
        {isReadyOrDelivered && transaction.status !== 'DELIVERED' && (
          <View style={styles.actions}>
            <Button
