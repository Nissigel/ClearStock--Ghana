...
export default function PaymentSuccessScreen() {
  // amount/sellerName come from the old manual-entry payment flow;
  // requestId comes from the newer purchase-request payment flow. Both
  // land on this same screen, so every param here is optional.
  const { reference, amount, sellerName, requestId } = useLocalSearchParams<{
    reference: string;
    amount?: string;
    sellerName?: string;
    requestId?: string;
  ...
  const handleViewOrder = () => {
    if (requestId) {
      ...
      router.replace('/(buyer)/(screens)/transactions');
    }
  };
